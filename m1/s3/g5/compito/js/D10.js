...
/* ESERCIZIO E
  Crea del codice per aggiungere programmaticamente all'oggetto precedentemente creato un 
  array chiamato "skills", contenente i linguaggi di programmazione che conosci.
*/
...
function deleteOne (stringa, boo) {
  if (boo) {
...
console.log(deleteOne('Ciao sono una stringa', true));
...
console.log(onlyLetters('I have 2 cats. Their names are Byron and Eliot.'));
...
function isThisAnEmail (stringa) {
  let mail = /[A-z0-9.+-]+@[A-z0-9.-]+.[A-z]{2,6}/;
  return mail.test(stringa);
}
console.log(isThisAnEmail('alice.inwonderland@gmail.com'));
...
function howManyDays (data) {
  ...
  let daysPassed = Math.round(Math.abs((date-today) / day));
  return daysPassed;
}

console.log(howManyDays('1995/09/02'));
...
function todayIsMyBirthday (comple) {
  let today = new Date();
  let birthday = new Date(comple);
...
console.log(todayIsMyBirthday('1995/09/02'));
...
function newestMovie(movies) {
  ...
  for (let film of movies) {
    if (!newerMovie || film.Year > newerMovie.Year) {
      newerMovie = film;
    }
  }
  return newerMovie;
...
/* ESERCIZIO 15
  Scrivi una funzione chiamata "onlyInLastMillennium" che 
  ritorna solamente i film prodotto nel millennio scorso contenuti nell'array "movies" fornito.
*/

function onlyInTheLastMillennium (movies) {
  let lastMill = movies.filter(movie => movie.Year <= 1999);
  return lastMill;
}
console.log(onlyInTheLastMillennium(movies));
...
function searchByTitle (stringa) {
  let string = movies.filter(movie => 
...
function removeIndex (number) {
  let index = movies.splice(number, 1);
  return movies;
}
...
function redBg () {
  let linkBg = document.querySelectorAll('a');
  for (let a of linkBg) {
...
/* ESERCIZIO 27
  Crea una funzione chiamata "halfTree" che riceve un numero come parametro e costruisce 
  un mezzo albero di "*" (asterischi) dell'altezza fornita.
